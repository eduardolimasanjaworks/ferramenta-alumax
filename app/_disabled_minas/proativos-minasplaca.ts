...
import { obterStatusConexao, enviarTextoSimples } from './lib/evolution.js';
...
async function enviarProativo(ctx: ContextoEnvio): Promise<void> {
  ...
  if (!status.conectado) {
    await registrarLogDisparo({
      ...
      id_alvo: ctx.idAlvo,
      status: 'erro',
      erro: `evolution desconectada (${status.state ?? 'unknown'})`,
    });
    return;
  }
  ...
  try {
    await enviarTextoSimples(config.evolutionInstance, telefoneDestino, ctx.mensagem);
