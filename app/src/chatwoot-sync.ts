...
export async function sincronizarTelefoneRapido(
  ...
  const { obterEstadoPausa, definirPausa, validarTelefone, IaBloqueadaPorWhatsappError } = await import('./pausa-minasplaca.js');
  ...
  try {
    ...
    if (err instanceof IaBloqueadaPorWhatsappError) {
      return {
        ok: true,
        estado: estadoAtual ?? { pausada: redisPausada, telefone: n, atualizado_em: new Date().toISOString() },
        alterado: false,
        status_ia: cw.status_ia,
      };
    }
...
export async function reconciliarSincroniaPausas(
  ...
  const { listarPausasAtivas, obterEstadoPausa, definirPausa, obterLogsPausa, IaBloqueadaPorWhatsappError } = await import('./pausa-minasplaca.js');
  ...
  for (const telefone of watch) {
    ...
    if (statusIa) {
      try {
        ...
        const bloqueado = err instanceof IaBloqueadaPorWhatsappError;
        resultados.push({
          telefone,
          redis_pausada: redisPausada,
          chatwoot_status_ia: statusIa,
          acao: bloqueado ? 'ok' : 'erro',
          detalhe: bloqueado
            ? 'WhatsApp desconectado — sync de religar adiado'
            : detalhe,
        });
      }
      continue;
