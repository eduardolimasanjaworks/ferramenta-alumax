...
export async function criarUsuarioChatwoot(dados: {
  ...
  try {
    ...
    if (!create.ok || !body.id) {
      // E-mail já existe: tenta reutilizar agent da account e só vincular.
      const { buscarAgentPorEmail } = await import('./chatwoot-agents.js');
      const existente = await buscarAgentPorEmail(email);
      if (existente) {
        const link = await platformFetch(
          `/platform/api/v1/accounts/${config.chatwootAccountId}/account_users`,
          {
            method: 'POST',
            body: JSON.stringify({ user_id: existente.id, role }),
          },
        );
        if (link.ok || link.status === 422) {
          if (dados.senha) {
            await platformFetch(`/platform/api/v1/users/${existente.id}`, {
              method: 'PATCH',
              body: JSON.stringify({ password: dados.senha, name: dados.nome }),
            }).catch(() => undefined);
          }
          return {
            ok: true,
            userId: existente.id,
            motivo: `reusou_existente_${existente.id}`,
          };
        }
      }
      return {
...
/**
 * Remove usuário da account Tilit + apaga no Platform.
 * Idempotente: 404 em qualquer passo ainda conta como ok.
 */
export async function excluirUsuarioChatwoot(
  chatwootUserId: number,
): Promise<{ ok: boolean; motivo?: string }> {
  if (!platformToken()) {
    return { ok: false, motivo: 'token_plataforma_ausente' };
  }

  const passos: string[] = [];

  try {
    const unlink = await platformFetch(
      `/platform/api/v1/accounts/${config.chatwootAccountId}/account_users`,
      {
        method: 'DELETE',
    ...
    if (unlink.ok || unlink.status === 404) {
      passos.push('account_user_ok');
    } else {
      passos.push(`account_user_http_${unlink.status}`);
    }
  } catch (err) {
    passos.push(`account_user_err:${err instanceof Error ? err.message : String(err)}`);
  }

  try {
    const del = await platformFetch(`/platform/api/v1/users/${chatwootUserId}`, {
      method: 'DELETE',
    });
    if (del.ok || del.status === 404) {
      passos.push('user_ok');
      console.log(
        `[chatwoot-usuarios] user ${chatwootUserId} removido da account ${config.chatwootAccountId}`,
      );
      return { ok: true, motivo: passos.join('|') };
    }
    const detail = await del.text().catch(() => '');
    return {
      ok: false,
      motivo: `user_delete_http_${del.status}: ${detail.slice(0, 180)} | ${passos.join('|')}`,
    };
  } catch (err) {
    return {
      ok: false,
      motivo: `${err instanceof Error ? err.message : String(err)} | ${passos.join('|')}`,
    };
  }
}
...
export { garantirPermissaoSso, ssoUrlParaUsuario } from './chatwoot-sso-login.js';
