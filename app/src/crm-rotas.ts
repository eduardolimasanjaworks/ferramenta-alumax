...
import { enviarBoardJson } from './crm-board-http.js';
import {
  atualizarPipeline,
  criarPipeline,
  excluirPipeline,
  listarPipelines,
} from './crm-pipelines.js';
...
import { sincronizarResponsavelCrmParaAtendimento } from './crm-assignee-write.js';
import {
  normalizarBodyResponsavel,
  patchTemResponsavel,
} from './crm-assignee-normalize.js';
...
const poolCrm = new pg.Pool({ connectionString: config.databaseUrl });
...
export async function rotasCrm(app: FastifyInstance): Promise<void> {
  app.get('/api/crm/board', async (req, reply) => {
    ...
    const q = (req.query ?? {}) as { pipelineId?: string };
    const board = await obterBoard(q.pipelineId || null);
    return enviarBoardJson(req, reply, board);
  });

  /* Funis (pipelines) */
  app.get('/api/crm/pipelines', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const pipelines = await listarPipelines(poolCrm);
    return { ok: true, pipelines };
  });

  app.post('/api/crm/pipelines', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const body = (req.body ?? {}) as { nome?: string };
    if (!body.nome?.trim()) {
      return reply.code(400).send({ ok: false, erro: 'nome obrigatorio' });
    }
    const pipeline = await criarPipeline(poolCrm, { nome: body.nome });
    return { ok: true, pipeline };
  });

  app.patch('/api/crm/pipelines/:id', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const { id } = req.params as { id: string };
    const body = (req.body ?? {}) as { nome?: string; principal?: boolean };
    const pipeline = await atualizarPipeline(poolCrm, id, body);
    if (!pipeline) return reply.code(404).send({ ok: false, erro: 'Funil nao encontrado' });
    return { ok: true, pipeline };
  });

  app.delete('/api/crm/pipelines/:id', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const { id } = req.params as { id: string };
    const body = (req.body ?? {}) as { moverParaId?: string };
    if (!body.moverParaId) {
      return reply.code(400).send({ ok: false, erro: 'moverParaId obrigatorio' });
    }
    const ok = await excluirPipeline(poolCrm, id, { moverParaId: body.moverParaId });
    if (!ok) {
      return reply.code(400).send({
        ok: false,
        erro: 'Nao foi possivel apagar (informe outro funil destino; precisa restar ao menos 1)',
      });
    }
    return { ok: true };
  });
  ...
  app.get('/api/crm/colunas', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const q = (req.query ?? {}) as { pipelineId?: string };
    return { ok: true, colunas: await listarColunas(q.pipelineId || null) };
  });

  app.post('/api/crm/colunas', async (req, reply) => {
    if (!(await exigirAuth(req, reply))) return;
    const body = (req.body ?? {}) as { titulo?: string; cor?: string; pipelineId?: string };
  ...
  app.patch('/api/crm/contatos/:id', async (req, reply) => {
    ...
    try {
      ...
      // Normaliza responsável (id do painel e/ou nome) antes de persistir
      const mudouResponsavel = patchTemResponsavel(body);
      if (mudouResponsavel) await normalizarBodyResponsavel(body);
      ...
      if (
        mudouResponsavel &&
        (antes.responsavel !== contato.responsavel ||
          antes.responsavelUsuarioId !== contato.responsavelUsuarioId)
      ) {
        void sincronizarResponsavelCrmParaAtendimento(contato)
          .then((r) => {
            if (!r.ok && r.motivo !== 'sem_responsavel') {
              console.warn('[crm] assignee→atendimento:', r.motivo);
            }
          })
          .catch((err) => console.error('[crm] assignee sync:', err));
      }
