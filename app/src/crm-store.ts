...
import { carregarBoardBatched, type CrmContatoBoard } from './crm-board-load.js';
import { invalidateBoardCache, obterBoardComCache } from './crm-board-http.js';
import { colunaEntradaPrincipal, garantirSchemaPipelines } from './crm-pipelines.js';
...
export type CrmContato = {
  ...
  anotacoes: string;
  /** Nome do atendente responsável (espelha assignee do Atendimento). */
  responsavel: string;
  /** ID em painel_usuarios; null = sem responsável. */
...
export async function inicializarBancoCrm(): Promise<void> {
  ...
  await pool.query(`
    ...
    ALTER TABLE crm_contatos ADD COLUMN IF NOT EXISTS responsavel_usuario_id INTEGER;
  ...
  await garantirSchemaPipelines(pool);
}
...
export async function obterBoard(pipelineId?: string | null): Promise<{
  colunas: CrmColuna[];
  contatos: CrmContatoBoard[];
}> {
  const pid = pipelineId || null;
  return obterBoardComCache(pid, () => carregarBoardBatched(pool, urlArquivo, pid));
}

export async function listarColunas(pipelineId?: string | null): Promise<CrmColuna[]> {
  if (pipelineId) {
    const r = await pool.query(
      `SELECT id, titulo, cor, ordem, pipeline_id AS "pipelineId"
       FROM crm_colunas WHERE pipeline_id = $1 ORDER BY ordem ASC`,
      [pipelineId],
    );
    return r.rows.map((row) => ({
      ...
      pipelineId: row.pipelineId ? String(row.pipelineId) : undefined,
    }));
  }
  const r = await pool.query(
    `SELECT id, titulo, cor, ordem, pipeline_id AS "pipelineId"
     FROM crm_colunas ORDER BY ordem ASC`,
  );
  return r.rows.map((row) => ({
    id: String(row.id),
    titulo: String(row.titulo),
...
export async function criarColuna(dados: {
  titulo?: string;
  cor?: string;
  pipelineId?: string;
}): Promise<CrmColuna> {
  invalidateBoardCache();
  let pipelineId = dados.pipelineId;
  if (!pipelineId) {
  ...
  if (!pipelineId) throw new Error('Nenhum funil CRM disponível');

  const { rows } = await pool.query<{ m: number }>(
    `SELECT COALESCE(MAX(ordem), -1) + 1 AS m FROM crm_colunas WHERE pipeline_id = $1`,
    [pipelineId],
  );
  ...
  await pool.query(
    ...
    [col.id, col.titulo, col.cor, col.ordem, pipelineId],
  );
  return col;
}
...
export async function reordenarColunas(origemId: string, destinoId: string): Promise<CrmColuna[]> {
  invalidateBoardCache();
  const { rows: meta } = await pool.query<{ pipeline_id: string | null }>(
    `SELECT pipeline_id FROM crm_colunas WHERE id = $1`,
    [origemId],
  );
  const pid = meta[0]?.pipeline_id ?? null;
  const cols = await listarColunas(pid);
...
export async function upsertContatoFromChatwoot(dados: {
  ...
  const cols = await listarColunas();
  // Entrada WhatsApp → 1ª coluna do funil principal
  const colunaId =
    (await colunaEntradaPrincipal(pool)) || cols[0]?.id || COLUNAS_PADRAO[0].id;
  const id = uid('ct');
  const agora = new Date().toISOString();
  await pool.query(
    `INSERT INTO crm_contatos
...
export async function atualizarContato(
  ...
  await pool.query(
    `UPDATE crm_contatos SET
      ...
      automacao_ativa = $11,
      responsavel_multichat = $12,
      responsavel_usuario_id = $13,
      atualizado_em = NOW()
     WHERE id = $1`,
    [
      id,
      ...
      merged.responsavel ?? '',
      merged.responsavelUsuarioId ?? null,
    ],
  );
...
export async function definirResponsavelContato(
  id: string,
  dados: { responsavel: string; responsavelUsuarioId: number | null },
): Promise<CrmContato | null> {
  const r = await pool.query(`SELECT id FROM crm_contatos WHERE id = $1`, [id]);
  if (!r.rows[0]) return null;
  await pool.query(
    `UPDATE crm_contatos SET
      responsavel_multichat = $2,
      responsavel_usuario_id = $3,
      atualizado_em = NOW()
     WHERE id = $1`,
    [id, dados.responsavel || '', dados.responsavelUsuarioId],
  );
  return obterContato(id);
}
