...
import { enviarTextoSimples } from './lib/evolution.js';
...
export async function notificarIntervencaoHumana(params: {
  ...
  if (process.env.TRANSFER_STUB_NOTIF === '1') {
    return { enviado: true, motivo: 'stub' };
  }
  ...
  try {
    await enviarTextoSimples(config.evolutionInstance, destino, texto);
