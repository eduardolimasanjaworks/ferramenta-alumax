...
import { obterStatusConexao } from './lib/evolution.js';
import { uazStatus } from './lib/uazapi.js';
import { listaInstanciasWa } from './lib/whatsapp-instancias.js';
...
export async function whatsappConectadoParaIa(): Promise<boolean> {
  if (config.uazapiToken && config.uazapiBaseUrl) {
    try {
      const uaz = await uazStatus();
      if (uaz.conectado) return true;
    } catch {
      /* tenta Evolution abaixo */
    }
  }
  for (const inst of listaInstanciasWa().filter((i) => i.provider === 'evolution')) {
    try {
      const status = await obterStatusConexao(inst.name);
      if (status.conectado === true) return true;
    } catch {
      /* próxima */
    }
  }
  const status = await obterStatusConexao(config.evolutionInstance);
...
export async function definirPausa(
  ...
  if (process.env.TRANSFER_STUB_PAUSA === '1') {
    const n = String(telefone || '').replace(/\D/g, '') || 'stub';
    return {
      pausada: pausar,
      telefone: n,
      status: meta.status,
      motivo: meta.motivo,
      origem: meta.origem ?? 'stub',
