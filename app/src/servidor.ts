...
const ORIGENS_CORS = new Set([
  config.publicUrl,
  config.publicUrl.replace(/^https:/, 'http:'),
]);
...
export async function criarServidor() {
  ...
  app.post('/api/auth/alterar-senha', async (req, reply) => {
    ...
    try {
      // Painel primeiro: falha no Atendimento NAO desfaz o login do painel
      await alterarSenhaUsuario(u.id, atual, nova);
      let atendimento: { ok: boolean; motivo?: string } | null = null;
      if (u.chatwoot_user_id) {
        const r = await atualizarUsuarioChatwoot(u.chatwoot_user_id, { senha: nova });
        atendimento = {
          ok: r.ok,
          motivo: mensagemPublicaAtendimento(r.motivo || (r.ok ? 'ok' : 'falha')),
        };
      }
      return { ok: true, mensagem: 'Senha alterada com sucesso', atendimento };
    } catch (err) {
  ...
  /** Flags de UI (calendário etc.) — phone.html esconde menu conforme isso. */
  app.get('/api/ui-flags', async () => ({
    ok: true,
    calendario: config.calendarioHabilitado,
  }));
