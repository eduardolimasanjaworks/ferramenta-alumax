 ...
 * Senha de login em hash; cofre cifrado separado para o admin copiar.
 */
import pg from 'pg';
import { config } from './config.js';
import { cifrarSenhaCofre } from './credenciais-cofre.js';
import { reconciliarAdminDoEnv } from './usuarios-admin-bootstrap.js';
import { hashSenha, verificarSenha } from './usuarios-senha.js';

export { alterarSenhaUsuario, obterCredenciaisParaCopia } from './usuarios-senha-ops.js';
...
function rowParaUsuario(row: Record<string, unknown>): UsuarioPainel {
  ...
  return {
    ...
    tem_cofre: Boolean(row.senha_cofre),
...
export async function inicializarBancoUsuarios(): Promise<void> {
  ...
  `);
  await pool.query(`ALTER TABLE painel_usuarios ADD COLUMN IF NOT EXISTS senha_cofre TEXT`);
  ...
  if (Number(rows[0]?.n || 0) === 0) {
    ...
    await pool.query(
      `INSERT INTO painel_usuarios (email, nome, senha_hash, senha_cofre, role, abas, ativo)
       VALUES ($1, $2, $3, $4, 'admin', $5::jsonb, true)`,
      [email, 'Administrador', hashSenha(senha), cifrarSenhaCofre(senha), JSON.stringify(todas)],
    );
    console.log(`[usuarios] Admin seed criado: ${email}`);
  }
  // Sempre: hash/cofre do admin do env — nunca depende de senha do Atendimento
  await reconciliarAdminDoEnv();
}
...
export async function obterUsuarioPorEmail(email: string): Promise<UsuarioPainel | null> {
  const e = email.trim().toLowerCase();
  if (!e) return null;
  const res = await pool.query('SELECT * FROM painel_usuarios WHERE lower(email) = lower($1) LIMIT 1', [e]);
  if (!res.rows[0]) return null;
...
/** Lookup pelo id do agent no Atendimento. */
export async function obterUsuarioPorChatwootId(
  chatwootUserId: number,
): Promise<UsuarioPainel | null> {
  const res = await pool.query(
    'SELECT * FROM painel_usuarios WHERE chatwoot_user_id = $1 LIMIT 1',
    [chatwootUserId],
  );
  if (!res.rows[0]) return null;
  return rowParaUsuario(res.rows[0]);
...
export async function criarUsuario(dados: {
  ...
  try {
    const res = await pool.query(
      `INSERT INTO painel_usuarios (email, nome, senha_hash, senha_cofre, role, abas, ativo, chatwoot_user_id)
...
export async function atualizarUsuario(
  ...
  const res = await pool.query(
    `UPDATE painel_usuarios SET
       ...
       senha_cofre = COALESCE($8, senha_cofre),
       atualizado_em = NOW()
     WHERE id = $1
     RETURNING *`,
    [id, nome, role, JSON.stringify(abas), ativo, chatwoot, senhaHash, senhaCofre],
  );
  return paraPublico(rowParaUsuario(res.rows[0]));
}
...
export async function excluirUsuario(id: number, adminId: number): Promise<void> {
  ...
  await excluirUsuarioSistema(id);
}

/** Exclusão por sync (agent sumiu no Atendimento) — não exige admin da sessão. */
export async function excluirUsuarioSistema(id: number): Promise<void> {
