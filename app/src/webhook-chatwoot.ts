...
import { cancelarFollowup, reagendarSeUltimoFoiLadoTilit } from './followup-minasplaca.js';
...
import { aplicarAssigneeDoPayloadAtendimento } from './crm-assignee-inbound.js';
...
export async function rotasWebhookChatwoot(app: FastifyInstance): Promise<void> {
  ...
  app.post('/webhook/chatwoot', async (req, reply) => {
    ...
    // Espelha contato + assignee Atendimento → card CRM (não bloqueia pausa)
    void upsertContatoDoPayloadChatwoot(payload).catch((err) => {
      console.error('[webhook-chatwoot] sync CRM falhou:', err);
    });
    void aplicarAssigneeDoPayloadAtendimento(payload).catch((err) => {
      console.error('[webhook-chatwoot] sync assignee CRM falhou:', err);
    });
    ...
    if (pausar) {
      ...
      void reagendarSeUltimoFoiLadoTilit(telefone).catch(() => {});
    }
  ...
  app.post('/api/ia/sincronizar-pausas', async (req, reply) => {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn('[sincronizar-pausas]', msg);
      return reply.status(200).send({
        ok: false,
        total: 0,
        corrigidos: 0,
  ...
  app.get('/api/ia/sincronizar-pausas', async (req, reply) => {
    try {
      ...
      const mensagens = [
        {
          ...
          content:
            'Você é o orquestrador do modo treinador da Tilit. Fale de forma informal, natural e prestativa, como um copiloto técnico experiente. ' +
