...
console.log(`\n🔍 Verificando webhook atual no Evolution API (${instance})...\n`);
...
async function checarWebhook() {
  try {
    ...
    console.log('Webhook Configurado Atual:', JSON.stringify(data, null, 2));
  } catch (err) {
    console.error('❌ Erro ao checar webhook:', err.message);
  }
}
