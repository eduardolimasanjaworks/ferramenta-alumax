#!/bin/bash
# Bateria de saúde — Tilit (iatilit)
set -uo pipefail

BASE="${BASE_URL:-http://127.0.0.1:8096}"
EXT="${EXT_URL:-https://iatilit.sanjaworks.com}"
EMAIL="${ADMIN_EMAIL:-admin@tilitgroup.com}"
...
for svc in tilit_app tilit_postgres tilit_redis; do
  docker ps --format '{{.Names}}' | grep -qx "$svc" && ok "$svc rodando" || fail "$svc ausente"
done
docker inspect tilit_postgres --format '{{.State.Health.Status}}' 2>/dev/null | grep -q healthy && ok "Postgres healthy" || fail "Postgres unhealthy"
docker inspect tilit_redis --format '{{.State.Health.Status}}' 2>/dev/null | grep -q healthy && ok "Redis healthy" || fail "Redis unhealthy"
...
section "5. Postgres"
counts=$(docker exec tilit_postgres psql -U tilit -d tilit -t -A -c "
...
node -e "const fs=require('fs');const h=fs.readFileSync('/root/tilit-rag/ferramenta-tilit/app/public/phone.html','utf8');new Function(h.match(/<script>([\\s\\S]*?)<\\/script>/)[1]);" 2>/dev/null \
  && ok "JS do painel válido" || fail "JS do painel inválido"
(cd /root/tilit-rag/ferramenta-tilit/app && node ./node_modules/typescript/bin/tsc --noEmit 2>/dev/null) \
  && ok "tsc --noEmit" || fail "tsc com erros"

section "7. Calculadora (no container)"
out=$(docker exec tilit_app node --input-type=module -e "
  -H 'Content-Type: application/json' -d '{}')
[ "$code" != "401" ] && ok "Webhook Chatwoot autenticado (HTTP $code)" || fail "Webhook sem auth"

section "9. Logs do app"
docker logs tilit_app --since 30m 2>&1 | rg -qi "fatal|Falha fatal" \
